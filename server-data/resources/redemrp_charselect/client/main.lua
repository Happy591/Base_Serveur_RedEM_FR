...
OpenCharacterMenu = function()
    ...
    table.insert(elements, {label = "<strong>Créer un nouveau personnage</strong>", desc = "Créer un nouveau personnage.", image="items/character_new.png", value = "new"})
    MenuData.Open('default', GetCurrentResourceName(), 'charselect', {
        title    = "REDEM:RP 2023",
        subtext  = "Sélection de personnage",
        align    = 'top-right',
        opacity  = '0.95',
        elements = elements,
    },
    function(data, menu)
        if data.current.value == "new" then
            if #CharsList < 4 then
                ...
                RedEM.Functions.NotifyLeft("Limite de personnage atteinte!", "Vous ne pouvez pas avoir plus de 4 personnage!", "menu_textures", "menu_icon_alert", 3000)
            end
        else
            print("Sélectionner un personnage "..data.current.value)
            SelectingChar = data.current.value
            local elements = {
                {label = "Sélectionner le personnage", desc = "Spawn avec ce personnage?", image="items/charsel_confirm.png",value = "select"},
                {label = "Supprimer le personnage", desc = "Supprimer ce personnage?", image="items/charsel_delete.png", value = "delete"}
            }
            ...
            function(data, menu)
                ...
                elseif data.current.value == "delete" then
                    local elements = {
                        {label = "<span style=\"color:red;\">Confirmer la suppréssion</span>", desc = "Confirmer la suppréssion de ce personnage?", image="items/charsel_confirm.png", value = "confirm"},
                        {label = "Annuler", desc = "Annuler la suppréssion", image="items/charsel_cancel.png", value = "cancel"}
                    }
                    MenuData.Open('default', GetCurrentResourceName(), 'charselect3', {
                        title    = "REDEM:RP",
                        subtext  = "Suppression de personnage",
                        align    = 'top-right',
                        opacity  = '0.95',
                        elements = elements,
                    },
                    function(data, menu)
                        if data.current.value == "confirm" then
                            ...
                            AddTextEntry("FMMC_MPM_TYP71", "\"confirmer\" pour supprimer le personnage ("..CharsList[charId].firstname.." "..CharsList[charId].lastname.."), ESC pour annuler")
...
RegisterCommand("logout", function(source, args)
    RedEM.TriggerCallback("redemrp_respawn:IsPlayerDead", function(isDead)
        if not isDead then
            ...
            RedEM.Functions.NotifyRight( "Vous ne pouvez pas vous déconnecter en étant mort.", 3000)
        end
    end)
end)
