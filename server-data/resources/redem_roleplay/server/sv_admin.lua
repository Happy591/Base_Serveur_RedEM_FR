    ...
    if user.getGroup() == "mod" or user.getGroup() == "admin" or user.getGroup() == "superadmin" then
        if args[1] ~= nil and args[2] == nil and args[3] == nil then
            if DoesEntityExist(GetPlayerPed(args[1])) then
                ...
                RedEM.Functions.NotifyRight(source, "Se téléporter à "..GetPlayerName(args[1]).." ["..args[1].."]", 3000)
            else
        ...
        RedEM.Functions.NotifyRight(source, "Pas assez de permissions.", 3000)
    end
end
    ...
    if user.getGroup() == "mod" or user.getGroup() == "admin" or user.getGroup() == "superadmin" then
        if args[1] ~= nil and args[2] == nil and args[3] == nil then
            if DoesEntityExist(GetPlayerPed(args[1])) then
                ...
                RedEM.Functions.NotifyRight(source, "Vous avez téléporté "..GetPlayerName(args[1]).." ["..args[1].."] a vous", 3000)
            else
                RedEM.Functions.NotifyRight(source, "Joueur Hors-Ligne?", 3000)
            end
        elseif args[1] ~= nil and args[2] ~= nil and args[3] ~= nil then
...
RegisterCommand("say", function(source, args)
    ...
    if user.getGroup() == "mod" or user.getGroup() == "admin" or user.getGroup() == "superadmin" then
        if args[1] and args[2] then
            ...
            RedEM.Functions.NotifyRight(source, "Envoyer le message à "..id..".")
        end
    else
        RedEM.Functions.NotifyRight(source, "Pas assez de permissions.", 3000)
    end
...
AddEventHandler("redemrp:setJob", function(source, id, job, grade, cb)
	...
	if _perm ~= 0 and (pg.GetGroup() ~= "admin" and pg.GetGroup() ~= "superadmin") then
		print(pg.getName() .. " - ID: " .. _perm .. " - A ESSAYER D'UTILISER UNE COMMANDE SANS EN AVOIR LA PERMISSION")
	else
		local _source = tonumber(id)
		local user = RedEM.GetPlayer(_source)
		if not user then
			RedEM.Functions.NotifyRight(_perm, "Joueur invalide!", 3000)
		else
			user.SetJob(job)
...
AddEventHandler("redemrp:removeBankMoney", function(source, id, count, cb)
	...
	local pg = RedEM.GetPlayer(_perm)
	if _perm ~= 0 and (pg.GetGroup() ~= "admin" and pg.GetGroup() ~= "superadmin") then
		print(pg.getName() .. " - With ID: " .. _perm .. " -  A ESSAYER D'UTILISER UNE COMMANDE SANS EN AVOIR LA PERMISSION")
	else
		local _source = tonumber(id)
		local user = RedEM.GetPlayer(_source)
		if not user then
