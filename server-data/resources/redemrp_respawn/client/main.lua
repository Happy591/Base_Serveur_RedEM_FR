        ...
        while IsPlayerDead(PlayerId()) and not revived do
            ...
            while timer >= GetGameTimer() and not revived do
                if revived == false then
                    ...
                    if not medicsAlerted then
                        DrawTxt("[~pa~ENTER~q~] ALERTER LES DOCTEURS", 0.50, 0.85, 0.5, 0.5, true, 255, 255, 255, 255, true)
                    else
                        DrawTxt("DOCTEURS ALERTE", 0.50, 0.85, 0.5, 0.5, true, 255, 255, 255, 255, true)
                    end
            ...
            while true do
                ...
                if not ConfirmingRespawn then
                    DrawTxt("Pressez E pour Respawn", 0.50, 0.45, 0.8, 0.8, true, 255, 255, 255, 255, true)
                else
                    DrawTxt("Êtes vous sûr de ~nrespawn?~n~(~pa~E~q~) Oui~n~(~pa~BACKSPACE~q~) Non", 0.50, 0.45, 0.8, 0.8, true, 255, 255, 255, 255, true)
                end
...
RegisterNetEvent("redemrp_respawn:respawnCoords", function(coords)
    ...
    NetworkResurrectLocalPlayer(coords.x, coords.y, coords.z, 59.95, true, false, false)    SetEntityCoordsNoOffset(ped, coords.x, coords.y, coords.z, false, false, false, true)
